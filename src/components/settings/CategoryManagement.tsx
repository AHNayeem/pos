...
export default function CategoryManagement({ initialCategories }: CategoryManagementProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      try {
        ...
        addToast(editingCategory ? "Failed to update category" : "Failed to create category", "error");
      }
    });
  };
  ...
  const handleArchive = async () => {
    ...
    startTransition(async () => {
      try {
        await CategoryService.archiveCategory(deleteTarget.id);
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Create/Edit Modal */}
      <Modal isOpen={isModalOpen} onClose={() => { setIsModalOpen(false); setFormData(emptyForm); setEditingCategory(null); }} className="sm:max-w-2xl">
        <div className="p-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              <div className="md:col-span-2">
                <label htmlFor="name" className="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">Category Name</label>
                <Input
                  type="text"
                  id="name"
                  name="name"
              ...
              <div>
                <label htmlFor="parentId" className="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">Parent Category</label>
                <Input
                  type="text"
                  id="parentId"
                  name="parentId"
            ...
            <div className="flex justify-end gap-3 pt-2">
              <Button variant="outline" type="button" onClick={() => { setIsModalOpen(false); setFormData(emptyForm); setEditingCategory(null); }}>
                Cancel
              </Button>
      ...
      <Modal isOpen={!!deleteTarget} onClose={() => setDeleteTarget(null)} className="sm:max-w-md">
        <div className="p-6">
          <h3 className="mb-2 text-lg font-semibold text-gray-900 dark:text-white">Deactivate Category</h3>
          <p className="mb-6 text-sm text-gray-500 dark:text-gray-400">
            Are you sure you want to deactivate <span className="font-medium">{deleteTarget?.name}</span>? This action can be reversed by editing the category.
          </p>
